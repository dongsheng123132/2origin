...
> | 模型 · 基线 | 本象 | 裸模型 | 向量 RAG |
> |---|---|---|---|
> | qwen-plus · S 级（2 万字，各 10 轮） | **92.5%** | 75.0% | 75.0% |
> | qwen-plus · M 级（9.5 万字，各 11 轮） | **98.9%** | 75.0% | 75.0% |
> | deepseek-v4-flash · M 级（**n=1，冒烟**） | **100.0%** | **37.5%** | 75.0% |
>
> **最有分量的一条是最后一行**：deepseek 裸跑只有 37.5%，比 qwen 差一半——两个模型的
> 基础状态能力根本不在一个档次。接上同一套状态机与门禁后，双双打到满分附近。
> **本象把水平悬殊的两个模型拉到了同一高度：状态正确性来自架构，不来自模型能力。**
>
> 基线从 2 万字拉到 9.5 万字，优势**不降反升**——向量 RAG 本该在长上下文时更有用，
> 结果纹丝不动。检索能找回文本，找不回「关键物品此刻在谁手上」：那个答案不在任何一段
> 原文里，是推演出来的。RAG 能把弱模型拉到 75% 那堵墙前，但越不过去。
>
