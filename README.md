...
> | deepseek-v4-flash · M 级 | **98.8% ± 3.95**（n=10） | 53.8% ± 32.6（n=10） | 32.1% ± 37.1（n=7） |
>
...
>
> **deepseek 上本象臂已跑满 n=10**（[Run #19](benchmark/shadowbench-w/results-log.md)）：
> **98.8% ± 3.95**，十轮全部 5/5 完成，九轮满分、一轮 87.5%。
...
> 恰恰说明这次量到的是能力，不是模板。
>
> ⚠️ 但**这仍不是跨模型结论**：对照臂那两列已作废、须重跑，所以这只是 A3 臂
> 自己的分布。「状态正确性来自架构而非模型能力」需要的是修好探询之后的三臂对照。
>
