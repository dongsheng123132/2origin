...
**状态：v0.1 · 协议草案 + 首批实验数据**（见 [ShadowBench-W 实验记录](benchmark/shadowbench-w/results-log.md)）

> **实验已收窄主张（2026-08-03，三臂各 10 轮）**：本象的价值经检验只在**一个**维度上成立——
> 状态正确率 75.0% → 92.5%（p=0.0001），而向量 RAG 在此维度**毫无改善**（二十轮全部 75.0%，零方差）。
> 正文一致性上与 RAG 无差异（p=0.9905），且贵 55%。
