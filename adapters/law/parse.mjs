 ...
 * 所以规则是：取**判决/裁定如下之前最后一个**、且 30 字内跟着书名号的那个引导词——
 * 裁判依据段永远紧贴裁判主文，这一条比任何关键词表都稳。
 */
...
/** 案号：（2026）沪0101刑初123号 → { 案号, 类型 }。也支持民辖等管辖裁定案号。 */
export function parseCaseNo(text) {
  const m = String(text ?? '').match(/[（(]\s*(\d{4})\s*[)）]\s*([^\s]{0,12}?)(刑|民|行)(初|终|再|申|辖)\s*(\d+)\s*号/)
  if (!m) return null
  return {
    案号: m[0].replace(/\s+/g, ''),
    年份: Number(m[1]),
