...
//
// 这个文件**不是** compiler/context-compiler.mjs 的 fork，不要照着 commit-compiler
// 那样去合并（task:merge-benchmark-lib 只合了输出侧，原因记在这里）：
//
//   核心的投影是领域无关的——按 id/name 在任务文本里出现、再沿 relations 扩一跳来选，
//   渲染成统一的 `id（名）k=v；k=v`。它不认识「主角」「同地点」「未回收伏笔」。
//   这里的投影恰恰全建立在这些叙事概念上：主角 + 与主角同地者 + 关键物品持有者，
//   分成人物 / 物品 / 地点 / 世界规则 / 伏笔五段呈现。小说 canon 里没有 relations.jsonl，
//   核心那条扩边根本无从走起。
//
//   要合并，就得给核心加一个 render/sections 钩子，让本文件把选取和渲染整个覆盖掉——
//   共享的只剩预算算术那三行，而核心那侧真正的使用者只有 Memory 方言一个。
//   那不是复用，是为了消灭重复而造的抽象。**投影本来就该按域各写各的**，
//   协议要统一的是事务与校验（已合并），不是每个域该给模型看什么。
