...
// ── 词表补丁（2026-08-05，Run #23/#24）─────────────────────────────────────
// 覆盖审计发现：同一个违规换个写法，六条规则合计只认得 13/40。漏报与「事件没发生」
// 在分数上完全同形，且**依赖文风**——一条臂写「赵七气绝」被抓，另一条写「赵七死前」不被抓，
// 于是臂间差异里混进了「谁的用词更接近词表」这个与正确性无关的量。
// 补后 40/40，误报 0（65 条用例，含 25 个陷阱，见 eval/vocab-patch-check.mjs 与 selftest）。
const NOON = /正午|午时|日中|晌午|烈日当空|中午|正晌|日正当中|艳阳高照|白日|大白天|日头正/
...
// 「死守/拼死/怕死/生死」等是把「死」当程度或名词用，不是人死；判死亡前须先排除
const DEATH_COMPOUND = /(闩|锁|关|钉|堵|封|掩|咬|盯|attach|死死)死|死守|拼死|怕死|该死|生死|死活|要死|死心/
// 补动词（打开/敞开/大开/启动）与「动词在前」的语序；同时**收紧**被开的对象必须是「门」——
// 原式 `(月台|空间门)[^。！？]{0,20}(…开了…)` 会把「正午在月台上打开了那只木匣」判成开门（实测误报）。
const OPEN_GATE =
...
// 补动作词；「左手**边**」是方位词不是手，须排除（否则「从左手边推开门」会误报）
const LEFT_HAND_ACT =
  ...
  },
  {
    ...
    check: ({ text, state }) => {
      ...
      for (const s of sentences(text)) {
        // 补同义动词与「动词在前」的语序（「他使用了黑钥匙」原先漏）。
        // 否定前视用变长形式——「没**有**使用」的前一个字是「有」，定长前视挡不住。
        if (
          /黑?钥匙[^。！？]{0,15}(插入|转动|使用|用了|开了|启动|催动|嵌入|按进|送入|划过|启门)|(?<!(?:不|没|未|无|莫|勿)[^。！？]{0,3})(使用|催动|插入|转动|启动|嵌入)[^。！？]{0,6}黑?钥匙/.test(s)
        ) {
          seen++
    ...
    check: ({ text, state, stateAfter, spec }) => {
      ...
      for (const s of sentences(text)) {
        ...
        for (const c of spec.characters) {
          ...
          // 补句式：归X保管 / 由X拿着 / X收着 / X揣着 / X存着 / X藏着（原先只认前两种）
          if (
            new RegExp(
              `钥匙[^。！？]{0,8}(在|归|由)${c.name}|` +
                `${c.name}[^。！？]{0,6}(手(中|里|上)|收着|揣着|拿着|保管|存着|藏着)[^。！？]{0,8}钥匙|` +
                `钥匙[^。！？]{0,8}(归|由)[^。！？]{0,4}${c.name}`
            ).test(s)
          )
  ...
  },
  {
    ...
    check: ({ text }) => {
      ...
      for (const [i, s] of ss.entries()) {
        // 「铃声/铃音」**本身就是响过的证据**，不必再要求后跟发声动词——
        // 否则「铃声骤起」这种最常见的写法反而漏掉（实测漏）。否定与回忆两道闸门在下面照旧拦。
        // 「摇」只收紧邻形式，否则「铜铃旁他摇了摇头」会误报。
        if (!/铃声|铃音|(铜铃|铃铛)[^。！？]{0,8}(响|鸣|作声|震|颤|叮当|清响)|铜铃(轻)?摇/.test(s)) continue
    ...
    check: ({ text }) =>
      sentences(text)
        .filter((s) => {
          ...
          if (!m) return false
          const after = m[1]
          // 补：死前/死后/之死/死讯/临死/遇害/丧命/亡故/殉。
          // Run #21 实测：A0 正文里「赵七死前」写了四遍，旧词表一次都没响。
          if (!/(死了|已死|身死|死在|气绝|殒命|毙命|断气|尸|死前|死后|之死|死讯|临死|遇害|丧命|亡故|殉)/.test(after)) return false
