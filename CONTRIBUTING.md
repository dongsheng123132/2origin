...
| **量化方言红队** | 运行 `node adapters/quant/benchmark.mjs`，再提交一个能导致重复记仓、幽灵订单漏报、陈旧行情穿透、终态漂移或错误闭合的最小 JSONL。**不要提交真实账号、策略秘密或券商凭据** |
| **外部方言** | 现有成熟方言与量化 draft 都出自本仓库作者。再来一个第三方自己的领域，如果也不用改核心，才真正说明「通用外壳 + 领域方言」不是我们自己出卷自己答 |
